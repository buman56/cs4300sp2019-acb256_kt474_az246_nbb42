...
def get_suggestions(q):
	...
	movie_index_to_name = {index:movie_name for index, movie_name in enumerate(df['MuseumName'])}
	movie_index_to_description = {index:desc for index, desc in enumerate(desc_data)}
	...
	for i in reversed(top_5_idx): 
		top_5.append((movie_index_to_name[i],sim[i],movie_index_to_description[i]))
